...
public class TelaCadastroComerciante extends JFrame {
    ...
    private void construirInterface() {
        add(
                EstiloReAlimenta.criarCabecalho("Cadastro de Comerciante", "Preencha os dados abaixo para criar sua conta", "🏪", this::voltarLogin),
                BorderLayout.NORTH
        );
        add(
                EstiloReAlimenta.criarAreaScroll(criarCardFormulario()),
                BorderLayout.CENTER
        );
    }
    ...
    private JPanel criarCardFormulario() {
        RoundedPanel card =
                new RoundedPanel(16, EstiloReAlimenta.BRANCO);
        card.setLayout(new BoxLayout(card, BoxLayout.Y_AXIS));
        card.setBorder(
                BorderFactory.createEmptyBorder(32, 36, 32, 36)
        );
        card.setPreferredSize(
                new Dimension(460, 650)
        );

        // NOME DO PROPRIETÁRIO
        card.add(
                EstiloReAlimenta.criarLabel("Nome do Proprietário")
        );
        card.add(Box.createVerticalStrut(5));
        campoProprietario = new JTextField();
        card.add(
                EstiloReAlimenta.criarCampoTexto(campoProprietario, "Digite seu nome completo", "assets/icons/user.png")
        );
        ...
        // NOME DA LOJA
        card.add(
                EstiloReAlimenta.criarLabel("Nome da Loja")
        );
        card.add(Box.createVerticalStrut(5));
        campoLoja = new JTextField();
        card.add(
                EstiloReAlimenta.criarCampoTexto(campoLoja, "Digite o nome da sua loja", "assets/icons/store.png")
        );
        card.add(Box.createVerticalStrut(13));

        // CNPJ
        card.add(
                EstiloReAlimenta.criarLabel("CNPJ")
        );
        card.add(Box.createVerticalStrut(5));
        campoCnpj = new JTextField();
        card.add(
                EstiloReAlimenta.criarCampoTexto(campoCnpj, "Digite seu CNPJ", "assets/icons/document.png")
        );
        card.add(Box.createVerticalStrut(13));
        ...
        card.add(
                EstiloReAlimenta.criarCampoTexto(campoEndereco, "Digite o endereço completo da loja", "assets/icons/location.png")
        );
        card.add(Box.createVerticalStrut(13));

        // TELEFONE
        card.add(
                EstiloReAlimenta.criarLabel("Telefone")
        );
        card.add(Box.createVerticalStrut(5));
        campoTelefone = new JTextField();
        card.add(
                EstiloReAlimenta.criarCampoTexto(campoTelefone, "Digite o telefone de contato", "assets/icons/phone.png")
        );
        ...
        // SENHA
        card.add(
                EstiloReAlimenta.criarLabel("Senha")
        );
        card.add(Box.createVerticalStrut(5));
        campoSenha = new JPasswordField();
        card.add(
                EstiloReAlimenta.criarCampoSenha(campoSenha, "Digite sua senha")
        );
        card.add(Box.createVerticalStrut(13));

        // CONFIRMAR SENHA
        card.add(
                EstiloReAlimenta.criarLabel("Confirmar Senha")
        );
        card.add(Box.createVerticalStrut(5));
        campoConfirmar = new JPasswordField();
        card.add(
                EstiloReAlimenta.criarCampoSenha(campoConfirmar, "Confirme sua senha")
        );
        card.add(Box.createVerticalStrut(22));

        // BOTÃO REGISTRAR
        card.add(
                EstiloReAlimenta.criarBotaoPrimario("Registrar", e -> realizarCadastro())
        );
